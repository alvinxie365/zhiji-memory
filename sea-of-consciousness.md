...
description: 识海 v2.0 — 四境完整体系（炼气/筑基/金丹/元婴）
triggers:
  - 每次对话结束自动炼化
  - 新session开始前prefetch
  - 用户问"还记得什么"
...
- 新域需要用户确认（sea_approve_frontier）后才开辟
...
**高质量对话（规则预筛）：**
用户/记住了/学会了/搞懂了/不对/不是这样/以后都/从来都/满意/不满意/谢谢/这就是/原来/我明白了/不要
...
1. **神魂写入守护**：当 SOUL.md 被修改时自动检测并提示用户确认
2. **识浪精华提炼**：定期从 raw_waves 中提炼精华（超过7天的 raw 可以做深度理解）
3. **跨session神识**：跨 session 追踪用户意图（同一个项目多次对话串联）
